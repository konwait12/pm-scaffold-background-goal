***

name: project-background-goal
description: Turn raw business material into a human-readable project background and goal document through a PM conversation, with optional project-specific baseline reading.
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

# 项目背景与目标

## 作用

这个 skill 帮产品经理回答四个问题：

1. 为什么现在要做这件事？
2. 现在是怎么做的，哪里出了问题？
3. 做成后要改变什么结果，怎样判断成功？
4. 哪些角色、约束和未知会影响判断？

它只处理业务背景、问题、目标和边界，不写用户旅程、功能清单、页面、字段、接口、架构或实现任务。它可以脱离 pipeline、registry、下游产物和其他 skill，直接在对话中使用。

## 输入与材料边界

只使用 PM 在**本次任务**实际提供的原始材料，以及 PM 在本次对话中的明确回复。原始材料可以是会议纪要、口述、聊天记录、邮件、PPT、PDF、图片、流程草图、系统现状或线下做法；无需预设文件类型的分析方法，重点是理解材料本身。

### 项目专属基线（可选）

如果本次工作有"项目专属基线材料"（例如飞书/钉钉/Notion 上的项目章程文档、决策记录、既有 PRD 等），可以在启动时由 PM 指定基线来源，执行 skill 前先实际读取原文，把原文拆为四类记录：

1. **基线明确决定**：必须改变本 skill 行为的约束；
2. **基线业务示例**：用于解释方法的示例，只在 PM 明确指定时才是本次输入；
3. **基线讨论/建议**：可作为待采纳做法，不能写成项目事实；
4. **AI 解读**：对基线的归纳，必须与前三类分开。

示例（飞书 CLI）：

```bash
lark-cli docs +fetch \
  --doc <PM-指定的-doc-id> \
  --doc-format markdown \
  --scope full \
  --format pretty
```

以命令返回的完整原文为准，并在治理伴随文件记录检索命令、文档 ID、检索日期和使用的章节/时间点。若 CLI 不可用、文档无法读取或只返回摘要，先停止在 `needs_user_input`，请 PM 提供原文或修复访问；不得用本文件、交接文档或模型记忆代替。

从基线"明确决定"落实执行协议：先用对话把"要做什么、为什么做、主题/框架和业务本质"说清楚，再处理措辞或实现细节；按阶段逐步产出，不假设一次对话就能得到最终需求文档；本 skill 必须能单点使用，拥有清楚的原始输入和独立输出；验证必须把原始材料作为输入，并与人工期望或独立参照进行比对，不能拿最终 PRD、已确认产物或基线示例替代原始材料。中间产物可以交给下一阶段，但必须保留其候选状态和来源边界。

不得用联网搜索、通用行业知识、模型记忆、其他项目文件、既有 PRD 或产物、计划、案例和 reference 示例来补充、校正或推断本项目事实。它们也不能作为验证材料。

基线中的业务示例不是本 skill 的默认需求，也不是内置测试夹具。若 PM 用它们测试，必须在本轮明确指定原文材料和测试目的；否则只能作为方法示例登记，不能出现在事实、目标、角色或成功判断中。

AI 可以归纳材料、判断项目类型、识别缺口并提出问题；所有推断必须与来源事实分开标识，且不能自行升级为业务事实或决定。材料不足或互相冲突时，保留原意并向 PM 提问。

## 参考文档加载规则

参考文档不是默认全部注入的上下文。执行本 skill 时，按阶段读取以下文件；读取后再继续该阶段，不要只凭文件名猜测内容。

| 阶段         | 必须读取                                                                 | 按需读取                                                                                                                                                      |
| ---------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 预检与分析      | `references/thinking-framework.md`、`references/source-handling.md`   | `references/background-4elements.md`（**材料缺量化数据或目标不可量化时必读**：内含基线数据取数路径引导与 SLA 候选值推导框架，不得直接跳到 UNKNOWN / 定性描述）                                               |
| 项目类型判断与澄清  | `references/background-by-type.md`、`references/question-patterns.md` | `references/elicitation-techniques.md`、`references/stakeholder-4class.md`、`references/stakeholder-power-interest.md`                                      |
| 起草主文档与治理文件 | `references/output-contract.md`                                      | `references/business-constraint-taxonomy.md`、`references/positioning-stxswort.md`、`references/planning-report.md`、`references/value-complexity-matrix.md` |
| AI Audit   | `references/audit-checklist.md`、`references/anti-patterns.md`        | `references/business-constraint-taxonomy.md`、`references/stakeholder-power-interest.md`                                                                   |
| PM/业务方评审   | `references/reviewer-checklist.md`                                   | 与本次待确认事项直接相关的 reference                                                                                                                                   |

如果按需文件与本次项目无关，不必读取。所有 reference 只提供分析、提问和审计方法，不是项目输入，不能加入案例细节或默认结论，也不能替代 PM 对事实、类型、目标和取舍的确认。

## 使用协议（来自 2026-08-27 会议约定）

按会议原文的执行协议，本 skill 必须遵守以下三条：

1. **渐进式产出** — 不假设一次对话就能得到最终背景目标。按 8 步循环**逐步推进**：先与 AI 谈清业务本质、起点/现状和期望结果，再进入范围、约束和衡量方式；不要在背景目标尚未说清时跳到 PRD、页面或实现细节。
2. **单点验证** — 本 skill 拥有清楚独立的输入和输出；**可以单独运行并验证**，不依赖 pipeline、registry、下游产物或其他 skill。验证时只用 PM 指定的原始材料，并与 PM 人工期望或独立参照对比。
3. **用具体案例反复跑** — 拿一个真实业务案例跑本 skill,看产物是否清晰；如果不符合预期,**调整 skill 再跑**，直到产物能直接说出"为什么做、怎么做、做成什么样、谁/约束/未知"。这是会议原文强调的迭代闭环。

## 对话流程

### 1. 预检材料

先理解材料描述的高层业务事件：什么条件触发、谁参与、经历哪些关键步骤、产生什么结果。业务方给出的往往只是 PPT、粗略流程、功能点或口述；先把这件事讲清楚，再分析它的背景和目标。若"要做什么、为什么做、目标结果"仍不清楚，先输出预检解读和问题，不起草完整背景目标。

先向 PM 输出一份简短的材料解读，严格区分：

- 来源明确说出的事实；

- AI 根据材料得出的高层理解，包括项目想建立或改变的业务能力；

- 业务方已经做出的决定、尚未验证的假设、互相冲突或完全未知的内容。

如果材料主要是功能方案、页面、流程或客户旅程，它能证明范围、规则、角色、约束和流程，但不自动证明业务问题、紧迫性或成功目标。将可合理推出的理解标为 AI 判断，将无法得出的背景和目标明确列为待确认。

如果只有一句功能请求，也不要直接扩写成项目背景；先进入澄清。

### 2. AI 先判断项目类型

只使用以下三种类型：

- **重构**：已有系统或稳定的人工做法，要改变现状和结果；

- **从 0 到 1**：还没有可用的目标业务能力，要把已有的线下/人工业务过程，或一个新业务服务，建立成可运行的能力；已有小程序、账号体系或其他渠道不改变这一判断；

- **迭代**：已有目标业务能力中的局部新增或调整，目标是补一个明确的业务结果，不需要重新写完整项目章程。

先给出 AI 判断、证据、信心和可能误判点。然后必须向 PM 提供三选一：

我判断是【推荐类型】，依据是【证据】。请选择：重构 / 从 0 到 1 / 迭代。

以 PM 的选择为准，不能自动覆盖。若 PM 选择与 AI 不同，记录 PM 的理由到治理伴随文件。

### 3. 按类型澄清

每轮最多问 5 个高影响问题，优先问会改变类型、目标、角色、范围、成本、时间或风险的问题。问题要带上 AI 初步判断和影响，但不要用复杂表格逼 PM 填写。

**批处理 / 无 PM 在场模式（v1.1 · 源自盲测）**：单次批处理任务中 PM 无法实时答复时，按以下规则执行，不得卡死等待：frontmatter `project_type` 写 AI 判断值并在正文标注「待 PM 三选一确认」；`status` 取 `needs_user_input`；未答复的高影响问题全部落入「待确认与风险」与治理文件澄清记录，标注「待答复」及是否阻断；阻断项在摘要显式声明。PM 回流后再按对话模式收敛。

**多时点演进材料取舍规则（v1.1 · 源自盲测）**：同一主题存在多个时间版本的材料（如相隔数月的工作坊 deck）时，事实性内容以**最新版本为基准**，旧版与新版差异登记为 CONFLICT 或在治理文件记录演进脉络，不得并列采信互相矛盾的表述；单份材料内部的日期/数字不一致同样标 CONFLICT 并挂待确认，不得静默取其一。

提问顺序遵循会议的渐进式原则：先确认业务本质、起点/现状和期望结果，再进入范围、约束和衡量方式；不要在背景目标尚未说清时，跳到 PRD、页面或实现细节。每个问题给出 A/B/C 三个互斥选项（可加"其他"），由 PM 选择或改写，AI 不替 PM 做决定。

- **重构**：问清之前如何做、现状证据、最痛的问题、希望改变的结果，以及 before -> after 的差异。

- **从 0 到 1**：问清没有系统时线下如何做、分几步、谁参与、每步产出什么，以及要建立的业务结果。

- **迭代**：问清为什么现在加/改、具体要改变什么业务结果；背景目标保持短小，通常两三句话足够。

不确定但不阻断的内容可以标为"待确认"；会改变业务决定的内容必须停在 needs\_user\_input，等待 PM 回答。

### 4. 生成两份文件

材料足够后，生成同名文件：

- background-goal.md：人和 AI 都能直接阅读的主文档，只写自然语言背景、现状、问题、目标、角色、约束、边界和参考资料；

- background-goal.governance.md：给 AI、校验器和后续集成读取的治理伴随文件，保存类型判断、PM 选择、主张来源、知识状态、澄清记录、Audit、确认记录和主文档哈希。

主文档不能出现六态表、SRC 追溯矩阵、Constitution、ReviewRecord、哈希锚点或机器化问题寄存器。参考资料只用人可读的标题和链接/位置。

### 4.1 背景分析深层框架（v1.2 · 源自原材料共性分析）

当材料本身包含充足的品牌、业务或产品背景信息时，以下深层框架必须在起草阶段显式应用，不得直接跳到目标描述。

#### 4.1.1 三层背景叙事结构

背景分析不能只停留在"项目要做什么"，必须往上挖到品牌/业务动因，往下落到产品定位。默认按以下三层展开：

| 层次 | 内容 | 分析引导 |
|------|------|---------|
| **第一层 · Why（品牌/业务动因）** | 品牌理念、业务痛点、市场机会——为什么现在做这件事 | 问：材料有没有提到品牌承诺/业务痛点/市场变化？如果没有，标 UNKNOWN 并提问 |
| **第二层 · What（产品定位）** | 产品是什么、解决什么问题、核心价值主张 | 问：新材料给出了什么方案？它承诺了什么结果？ |
| **第三层 · Who（目标用户/角色）** | 主要用户角色、各角色的诉求、谁受益谁执行 | 问：哪些角色参与？哪些角色只影响不受益？ |

三层之间必须有因果链——业务痛点驱动产品定位，产品定位定义目标用户。材料缺失某一层时显式标注，不得用 AI 推断替代。

#### 4.1.2 双目标体系（体验目标 + 业务目标 + 产品目标）

目标分析不得只写单一"项目目标"。默认拆为三层目标，并明确因果链：

- **体验目标**（用户感知层面）：如"提升客户对作品的所有权感知"——关注用户感受到什么
- **业务目标**（业务结果层面）：如"提升客户复购率 / 售后渗透率"——关注业务指标变化
- **产品目标**（功能交付层面）：如"上线 DP 创建/查看/管理核心流程"——关注产品交付了什么

因果链方向：**产品目标 → 体验目标 → 业务目标**。上游材料常体现"通过提供品质服务 → 实现关系深化"的因果结构，产出物应复制这种逻辑。

#### 4.1.3 范围界定矩阵法

当材料涉及多个生命周期阶段和多个功能维度时，MVP 范围默认用**阶段 × 维度矩阵法**呈现，而非简单的 bullet list：

- **行**：生命周期阶段（触发 / 触达 / 首次使用 / 深度使用 / 管理 / 退出）
- **列**：功能维度（根据项目特性定义，如内容 / 服务 / 互动 / 通知 / 分享 / ……）
- **单元格标注**：MVP in / MVP out / 未来版本 / 待评估

矩阵法源自 CHANEL DP 项目 Client Journey PDF 和 IT Intake PDF 的共性实践，适用于多阶段多功能的复杂项目。单阶段单功能项目可声明"不适用"并记录理由。

#### 4.1.4 约束与假设分类清单

约束不能零散地散落在各处。必须在治理文件中按以下分类整理，每类列出具体约束项和来源：

| 约束类别 | 分析引导 | 示例 |
|---------|---------|------|
| **用户约束** | 谁能用、谁不能用、什么身份/等级有差异 | 必须绑定数字账户；未登录用户 x 行为受限 |
| **地域/法规约束** | 哪些地区可用、哪些法规影响设计 | 仅限国内购买；海外走 PCN 方案 |
| **技术约束** | 依赖哪些外部系统、有什么限制 | 依赖 Global API；CN 侧做缓存编排 |
| **业务约束** | 哪些场景暂不支持、哪些是已知的不会做 | Gifting 场景暂不覆盖 |
| **数据约束** | 哪些数据现在拿不到、哪些需要外部接入 | 部分字段依赖上游系统，延迟未知 |

约束缺失时标 UNKNOWN 并提问，不得默认"无约束"。

### 5. AI Audit

生成后，AI 以"项目组成员"的视角重新阅读主文档，回答：

- 我能否说清楚项目是什么、为什么做、要改变什么？

- 当前做法和核心问题是否有证据？

- 角色、约束和非目标是否遗漏？

- 目标是否能判断成败，或明确挂起到待确认事项？

- 是否把方案细节误写成业务事实？

将 Audit 结论写入治理伴随文件，不把审计表塞进主文档。

### 6. PM 确认

向 PM 展示摘要、关键证据、未知项、待决策项和 Audit 结果。只有 PM/业务事实负责人确认后，治理文件才可记录 confirmed；AI 不代替确认。

## 产出要求

主文档使用本 skill 自带模板（`templates/background-goal.md` 的标题，治理文件用 `templates/background-goal.governance.md`）。三种类型的最低要求：

| 类型      | 必须说清楚                         | 目标写法              |
| ------- | ----------------------------- | ----------------- |
| 重构      | 现状、问题证据、before -> after       | 尽量写基线、目标值、衡量方式和时间 |
| 从 0 到 1 | 已有人工做法或首个完整业务流程、参与角色、要建立的业务过程 | 写要建立的结果和验证方式      |
| 迭代      | 为什么加/改、加/改后做什么                | 简短、需求级，不膨胀成项目章程   |

目标可以暂时没有数字，但必须明确"如何判断"或链接到治理文件中的待确认问题；禁止凭空编造指标。

## 独立运行

直接对话即可完成本 skill。可选地运行：

```bash
python3 scripts/validate_artifact.py path/to/background-goal.md --json
```

校验器只依赖 Python 标准库；它会自动查找同目录的 `background-goal.governance.md`，不启动 pipeline，也不读取 registry 或下游文件。

## 禁止事项

- 不用成品 PRD 自证；

- 不把 AI 推断写成事实；

- 不替 PM 确认项目类型、范围、目标或取舍；

- 不把功能、页面、字段、API 或技术实现写进背景目标；

- 不为了"完整"复制一套固定长模板；

- 不把治理记录混入人读主文档。

